...
class is_unpackable {
  ...
  template <class U> static void check__ (tuple_size_t<U>*) noexcept;
  template <class> static void check__ (...) noexcept(false);
public:
  static constexpr bool value = noexcept(check__<T>(nullptr));
};
...
class is_runpackable {
  template <class U>
  static auto check__ (U* u) noexcept -> decltype(u->at(0ul), void());
  template <class> static void check__ (...) noexcept(false);
public:
  static constexpr bool value = noexcept(check__<T>(nullptr));
};
...
class is_swappable {
  ...
  static auto check__ (int) noexcept -> decltype(
    swap(declval<X&>(), declval<Y&>()),
    void()
  );
  template <class X, class Y> static void check__ (...) noexcept(false);
public:
  static constexpr bool value =
    noexcept(check__<T, U>(0)) and noexcept(check__<U, T>(0));
};
