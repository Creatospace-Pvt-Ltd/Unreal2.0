...
class is_unpackable {
  ...
  template <class U> static void __check (tuple_size_t<U>*) noexcept;
  template <class> static void __check (...) noexcept(false);
public:
  static constexpr bool value = noexcept(__check<T>(nullptr));
};
...
class is_runpackable {
  template <class U>
  static auto __check (U* u) noexcept -> decltype(u->at(0ul), void());
  template <class> static void __check (...) noexcept(false);
public:
  static constexpr bool value = noexcept(__check<T>(nullptr));
};
...
class is_swappable {
  ...
  static auto __check (int) noexcept -> decltype(
    swap(declval<X&>(), declval<Y&>()),
    void()
  );
  template <class X, class Y> static void __check (...) noexcept(false);
public:
  static constexpr bool value =
    noexcept(__check<T, U>(0)) and noexcept(__check<U, T>(0));
};
