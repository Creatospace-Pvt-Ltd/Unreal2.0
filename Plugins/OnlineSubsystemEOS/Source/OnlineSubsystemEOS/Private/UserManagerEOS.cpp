...
void FUserManagerEOS::CreateEOSVoiceRoomToken(FString clientID)
{
	...
	EOS_RTCAdmin_QueryJoinRoomToken(EOSSubsystem->RTCAdminHandle, &roomOptions, (void*)CallbackObj, CallbackObj->GetCallbackPtr());

}

void FUserManagerEOS::GetRoomToken(EOS_ProductUserId id, uint32_t query, const char* url)
{
	...
	EOS_RTCAdmin_CopyUserTokenByUserId(EOSSubsystem->RTCAdminHandle, &tokenOptions, token);
