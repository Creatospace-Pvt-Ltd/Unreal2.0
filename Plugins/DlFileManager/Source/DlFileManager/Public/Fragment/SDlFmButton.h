 ...
 */
class DLFILEMANAGER_API SDlFmButton : public SBorder
{
	SLATE_DECLARE_WIDGET(SDlFmButton, SBorder)
...
public:
	...
	SLATE_BEGIN_ARGS( SDlFmButton )
		: _Content()
		, _ButtonStyle( &FCoreStyle::Get().GetWidgetStyle< FButtonStyle >( "Button" ) )
		, _TextStyle( &FCoreStyle::Get().GetWidgetStyle< FTextBlockStyle >("ButtonText") )
		, _Paint_OverflowPercent(FVector2D::One())//Dl+
		, _Paint_OffsetPercent(FVector2D::Zero())//Dl+
		, _HAlign( HAlign_Fill )
		, _VAlign( VAlign_Fill )
		...
		/** Dl Feature */
		SLATE_ARGUMENT( FVector2D, Paint_OverflowPercent )//Dl+
		SLATE_ARGUMENT( FVector2D, Paint_OffsetPercent )//Dl+
...
private:
	...
	struct FDlPaintFeature//Dl+
	{//Dl+
		FVector2D Paint_OverflowPercent;//Dl+
		FVector2D Paint_OffsetPercent;//Dl+
		FGeometry Cached_Geometry;//Dl+
		FVector2D Cached_RealOffset;//Dl+
		FVector2D Cached_RealSize;//Dl+
		FSlateLayoutTransform Cached_RealTransform;//Dl+

		void Init(const FArguments& InArgs);//Dl+
		void Refresh(const FGeometry& AllottedGeometry);//Dl+
	};//Dl+
	FDlPaintFeature DlPaintFeature;//Dl+
	FDlPaintFeature* DlPaintFeature_Ptr=&DlPaintFeature;//Dl+
};
