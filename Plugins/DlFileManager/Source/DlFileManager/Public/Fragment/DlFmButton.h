...
public:
	...
	DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnButtonFilteredReleasedEvent);//Dl+
	DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnButtonFilteredClickedEvent);//Dl+
	DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnButtonFilteredPressedEvent);//Dl+
	
	/** Dl Additional Feature */
	UPROPERTY(EditAnywhere, Category = "Filter")//Dl+
	float GapTime = 0.1;//Dl+
	UPROPERTY(EditAnywhere, Category = "Filter")//Dl+
	FVector2D Paint_OverflowPercent = FVector2D::One();//Dl+
	UPROPERTY(EditAnywhere, Category = "Filter")//Dl+
	FVector2D Paint_OffsetPercent = FVector2D::Zero();//Dl+
...
public:
	/** Dl Additional Feature */
	UPROPERTY(BlueprintAssignable, Category = "Button|Event")//Dl+
	FOnButtonFilteredReleasedEvent OnFilteredReleased;//Dl+
	UPROPERTY(BlueprintAssignable, Category = "Button|Event")//Dl+
	FOnButtonFilteredClickedEvent OnFilteredClicked;//Dl+
	UPROPERTY(BlueprintAssignable, Category = "Button|Event")//Dl+
	FOnButtonFilteredPressedEvent OnFilteredPressed;//Dl+
...
protected:
	...
	TSharedPtr<SDlFmButton> MyButton;

private:
	
	/** Dl Additional Feature */
	float LastReleasedTime = 0;//Dl+
	float LastPressedTime = 0;//Dl+
	float LastClickedTime = 0;//Dl+
};
