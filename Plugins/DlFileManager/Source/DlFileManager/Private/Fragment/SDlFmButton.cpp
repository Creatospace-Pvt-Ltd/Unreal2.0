...
static FName SButtonTypeName("SDlFmButton");

SLATE_IMPLEMENT_WIDGET(SDlFmButton)
void SDlFmButton::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION_WITH_NAME(AttributeInitializer, "BaseBorderForegroundColor", BorderForegroundColorAttribute, EInvalidateWidgetReason::Paint)
		.OnValueChanged(FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateLambda([](SWidget& Widget)
			{
				static_cast<SDlFmButton&>(Widget).UpdateForegroundColor();
			}));
	...
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION_WITH_NAME(AttributeInitializer, "ContentPadding", ContentPaddingAttribute, EInvalidateWidgetReason::Layout)
		.OnValueChanged(FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateLambda([](SWidget& Widget)
		{
			static_cast<SDlFmButton&>(Widget).UpdatePadding();
		}));
	...
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION_WITH_NAME(AttributeInitializer, "AppearPressed", AppearPressedAttribute, EInvalidateWidgetReason::Paint)
		.OnValueChanged(FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateLambda([](SWidget& Widget)
			{
				static_cast<SDlFmButton&>(Widget).UpdatePressStateChanged();
			}));
	...
	AttributeInitializer.OverrideOnValueChanged("EnabledState"
		...
		, FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateLambda([](SWidget& Widget)
			{
				static_cast<SDlFmButton&>(Widget).UpdateBorderImage();
			}));
 ...
 */
void SDlFmButton::Construct( const FArguments& InArgs )
{
	DlPaintFeature.Init(InArgs);//Dl+
	...
	// Only do this if we're exactly an SDlFmButton
...
int32 SDlFmButton::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	DlPaintFeature_Ptr->Refresh(AllottedGeometry);//Dl+
	...
	if (BrushResource && BrushResource->DrawAs != ESlateBrushDrawType::NoDrawType)
	{
		FSlateDrawElement::MakeBox(
			OutDrawElements,
			LayerId,
			AllottedGeometry.ToPaintGeometry(DlPaintFeature.Cached_RealOffset,DlPaintFeature.Cached_RealSize,1),//Dl+
			BrushResource,
			DrawEffects,
...
FMargin SDlFmButton::GetCombinedPadding() const
{
	return ( IsPressed() )
...
void SDlFmButton::UpdatePadding()
{
	SetPadding(GetCombinedPadding());
}

bool SDlFmButton::GetShowDisabledEffect() const
{
	return Style->Disabled.DrawAs == ESlateBrushDrawType::NoDrawType;
}

//~ Update when { Style }
void SDlFmButton::UpdateShowDisabledEffect()
{
...
void SDlFmButton::UpdateBorderImage()
{
...
void SDlFmButton::UpdateForegroundColor()
{
...
//~ Update when { Style }
void SDlFmButton::UpdateDisabledForegroundColor()
{
	Invalidate(EInvalidateWidgetReason::Paint);
}

bool SDlFmButton::SupportsKeyboardFocus() const
{
	// Buttons are focusable by default
	return bIsFocusable;
}

void SDlFmButton::OnFocusLost( const FFocusEvent& InFocusEvent )
{
...
FReply SDlFmButton::OnKeyDown( const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent )
{
...
FReply SDlFmButton::OnKeyUp(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	FReply Reply = FReply::Unhandled();
...
FReply SDlFmButton::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	FReply Reply = FReply::Unhandled();
...
FReply SDlFmButton::OnMouseButtonDoubleClick(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SDlFmButton::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	FReply Reply = FReply::Unhandled();
...
FReply SDlFmButton::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
void SDlFmButton::OnMouseEnter( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
void SDlFmButton::OnMouseLeave( const FPointerEvent& MouseEvent )
{
	const bool bWasHovered = IsHovered();
...
void SDlFmButton::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
	Release();
}

FReply SDlFmButton::ExecuteOnClick()
{
...
void SDlFmButton::UpdatePressStateChanged()
{
	UpdatePadding();
	UpdateBorderImage();
...
bool SDlFmButton::IsInteractable() const
{
	return IsEnabled();
}

TEnumAsByte<EButtonClickMethod::Type> SDlFmButton::GetClickMethodFromInputType(const FPointerEvent& MouseEvent) const
{
...
bool SDlFmButton::IsPreciseTapOrClick(const FPointerEvent& MouseEvent) const
{
	return GetClickMethodFromInputType(MouseEvent) == EButtonClickMethod::PreciseClick;
}

void SDlFmButton::PlayPressedSound() const
{
	FSlateApplication::Get().PlaySound( PressedSound );
}

void SDlFmButton::PlayHoverSound() const
{
	FSlateApplication::Get().PlaySound( HoveredSound );
}

FVector2D SDlFmButton::ComputeDesiredSize(float LayoutScaleMultiplier) const
{
...
void SDlFmButton::SetContentPadding(TAttribute<FMargin> InContentPadding)
{
	ContentPaddingAttribute.Assign(*this, MoveTemp(InContentPadding));
}

void SDlFmButton::SetHoveredSound(TOptional<FSlateSound> InHoveredSound)
{
	HoveredSound = InHoveredSound.Get(Style->HoveredSlateSound);
}

void SDlFmButton::SetPressedSound(TOptional<FSlateSound> InPressedSound)
{
	PressedSound = InPressedSound.Get(Style->PressedSlateSound);
}

void SDlFmButton::SetOnClicked(FOnClicked InOnClicked)
{
	OnClicked = InOnClicked;
}

void SDlFmButton::SetOnHovered(FSimpleDelegate InOnHovered)
{
	OnHovered = InOnHovered;
}

void SDlFmButton::SetOnUnhovered(FSimpleDelegate InOnUnhovered)
{
	OnUnhovered = InOnUnhovered;
}

void SDlFmButton::ExecuteHoverStateChanged(bool bPlaySound)
{
	if (IsHovered())
	{
		if (bPlaySound)
...
void SDlFmButton::SetButtonStyle(const FButtonStyle* InButtonStyle)
{
...
void SDlFmButton::SetClickMethod(EButtonClickMethod::Type InClickMethod)
{
	ClickMethod = InClickMethod;
}

void SDlFmButton::SetTouchMethod(EButtonTouchMethod::Type InTouchMethod)
{
	TouchMethod = InTouchMethod;
}

void SDlFmButton::SetPressMethod(EButtonPressMethod::Type InPressMethod)
{
	PressMethod = InPressMethod;
}

#if !UE_BUILD_SHIPPING
void SDlFmButton::SimulateClick()
{
	ExecuteOnClick();
}
#endif // !UE_BUILD_SHIPPING

FSlateColor SDlFmButton::GetDisabledForegroundColor() const
{
	return Style->DisabledForeground;
}

#if WITH_ACCESSIBILITY
TSharedRef<FSlateAccessibleWidget> SDlFmButton::CreateAccessibleWidget()
{
...
void SDlFmButton::FDlPaintFeature::Init(const FArguments& InArgs)//Dl+
{//Dl+
	Paint_OffsetPercent=InArgs._Paint_OffsetPercent;//Dl+
	Paint_OverflowPercent=InArgs._Paint_OverflowPercent;//Dl+
}//Dl+

void SDlFmButton::FDlPaintFeature::Refresh(const FGeometry& AllottedGeometry)//Dl+
{//Dl+
	if(AllottedGeometry!=Cached_Geometry)//Dl+
	{//Dl+
		Cached_Geometry=AllottedGeometry;//Dl+
		Cached_RealSize = Cached_Geometry.GetLocalSize()*Paint_OverflowPercent;//Dl+
		Cached_RealOffset = Cached_Geometry.GetLocalSize()*(Paint_OffsetPercent-(Paint_OverflowPercent-1)/2);//Dl+
	}//Dl+
}//Dl+
