...
void custom_video_processor::handle_frame(std::unique_ptr<dolbyio::comms::video_frame> frame) {
  {
    std::lock_guard<std::mutex> lock(frame_queue_lock_);
...
void custom_video_processor::set_sink(const std::shared_ptr<video_sink>& sink, const config&) {
  {
