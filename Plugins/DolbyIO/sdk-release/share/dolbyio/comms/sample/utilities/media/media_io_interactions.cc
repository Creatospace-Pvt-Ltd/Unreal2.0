...
void media_io_interactions::initialize_injection() {
  ...
  bool audio = params_.override_inject_audio_.value_or(sdk_params_.conf.join_with_audio());
  bool video = params_.override_inject_video_.value_or(sdk_params_.conf.join_with_video());

  if (!audio && !video) {
    std::cerr << "No injection requested for audio or video, the input file will not be used" << std::endl;
    return;
  }

  if (!injector_) {
    injector_ =
        std::make_shared<plugin::injector_paced>([](const dolbyio::comms::plugin::media_injection_status& state) {
          std::cerr << "Media Injection Status Change ===> type: " << state.type_ << " state: " << state.state_
                    << " desc: " << state.description_ << std::endl;
        });
    if (video)
      sdk_params_.video_frame_handler = injector_;
  }
  if (!source_) {
    source_ = std::make_unique<file_source>(
        std::move(params_.files), params_.loop_the_injection_, *injector_,
        [this, audio, video](const dolbyio::comms::sample::file_source_status& status) {
            ...
            if (status.current_state == dolbyio::comms::sample::source_state::STOPPED) {
              if (audio)
                sdk_->audio()
                    ...
                    .on_error([](std::exception_ptr&&) { std::cerr << "Error stopping audio\n"; });
              if (video)
                sdk_->video()
                    .local()
                    .stop()
                    .then([]() { std::cerr << "video stopped\n"; })
                    .on_error([](std::exception_ptr&&) { std::cerr << "Error stopping video\n"; });
            }
          }
        });
  ...
  if (video)
    wait(sdk_->video().local().start(camera_device(), injector_));
}
...
void media_io_interactions::register_command_line_handlers(commands_handler& handler) {
  ...
  handler.add_command_line_switch(
      {"-iv"},
      "<true|false>\n\tOverride video injection setting (true - injecting video; false - not injecting video). The "
      "media_io will by default inject video if joining the conference with video, and will not inject otherwise.",
      [this](const std::string& arg) {
        ...
        if (arg == "true")
          ...
          std::cerr << "Invalid argument for the -iv option, using the defaults." << std::endl;
      });

  handler.add_command_line_switch({"-ia"},
                                  "<true|false>\n\tOverride audio injection setting (true - injecting audio; false - "
                                  "not injecting audio). The media_io will by default inject audio if joining the "
                                  ...
                                  [this](const std::string& arg) {
                                    ...
                                    if (arg == "true")
                                      ...
                                      std::cerr << "Invalid argument for the -ia option, using the defaults."
                                                << std::endl;
                                  });
}
