...
void ui_interface::print_create_conference_response() {
  std::cerr << "conference_info: DolbyVoice:" << sdk_inter->conference_info().dolby_voice
            << " LiveRecording:" << sdk_inter->conference_info().live_recording
            << " AudioOnly: " << sdk_inter->conference_info().audio_only << " RecordingFormats:";
  for (const auto& iter : sdk_inter->conference_info().recording_formats) {
    if (iter == recording_format::mp4)
      std::cerr << " mp4";
    else if (iter == recording_format::mp3)
      std::cerr << " mp3";
  }
  switch (sdk_inter->conference_info().rtcp_mode) {
    case dolbyio::comms::rtcp_mode::average:
      std::cerr << " rtcp_mode::average";
      break;
    case dolbyio::comms::rtcp_mode::max:
      std::cerr << " rtcp_mode::max";
      break;
    case dolbyio::comms::rtcp_mode::worst:
      std::cerr << " rtcp_mode::worst";
      break;
  }
  switch (sdk_inter->conference_info().video_codec) {
    case dolbyio::comms::video_codec::none:
      std::cerr << " video_codec::none";
      break;
    case dolbyio::comms::video_codec::h264:
      std::cerr << " video_codec::H264";
      break;
    case dolbyio::comms::video_codec::vp8:
      std::cerr << " video_codec::VP8";
      break;
  }
  std::cerr << std::endl;
}
  ...
  if (conf_params.id) {
    ...
    if (conf_params.alias->compare("DEMO") == 0) {
      sdk_inter->set_conference_info(
          wait(sdk_->conference().demo(sdk_inter->conference_options().params.spatial_audio_style)));
      joined = true;
      print_create_conference_response();
    } else {
      ...
      if (inject_inter->media_io_enabled() && sdk_params().conf.join_as_user()) {
        ...
        sdk_->audio()
            ...
            .on_error([](std::exception_ptr&&) {
              std::cerr << "Failed to set local audio to unprocessed, not starting injection!\n";
            });
      }
