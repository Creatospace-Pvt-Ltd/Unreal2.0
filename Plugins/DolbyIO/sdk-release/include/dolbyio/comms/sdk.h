...
class DLB_COMMS_EXPORT sdk {
 public:
  ...
  /**
   * @brief Registers the component version for internal telemetry.
   *
   * The component name and version are free-form strings, which will be added to the Dolby.io telemetry reports. If a
   * component with given name has already been registered, this method fails and returns error information
   * asynchronously.
   *
   * @return The result object producing the success or failure information asynchronously.
   *
   * \code{.cpp}
   * sdk->register_component_version("MyApp", "1.0.0")
   *      .on_error([](auto&&){ std::cerr << "Failed to register my app to telemetry" << std::endl; });
   * \endcode
   */
  virtual async_result<void> register_component_version(const std::string& name, const std::string& version) = 0;
