...
/**
 * @brief Partial specialization of the detail::async_result_with_solver template class, ensuring that the operations
 * are thread-safe.
 * @ingroup async_ops
 */
template <typename T>
...
/**
 * @deprecated Since 2.3. For optimal performance blocking until async_result is resolved is not recommended. However,
 * if needed it is the applications responsibility to wait for result of asynchronous operation to become available.
 *
 ...
 */
template <typename T>
#ifndef DOLBYIO_COMMS_ENABLE_DEPRECATED_WAIT
[[deprecated(
    "Blocking for asynchronous operations' resolution is dangerous. wait() used in any of the SDK's event callbacks or "
    "in result/error callbacks set on other async results will result in deadlocks. Blocking on the macos mainloop may "
    "deadlock against some SDK operations.")]]
#endif
