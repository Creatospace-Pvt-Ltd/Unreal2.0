...
class DLB_COMMS_MEDIA_EXPORT audio_device {
 public:
   ...
   *
   * The identity class is copyable and moveable, but it's mostly opaque. The application can not construct the identity
   * instances, and can not get any meaningful data out of the identity objects. This class serves a purpose of
   * identifying identical (same) devices in the platform-agnostic way, unline the system native ID.
   *
   * The identity class implements the equality and less-than comparison operators, and there's an std::hash<identity>
   * specialisation provided, so the identity class can be used as a key in any type of the std containers.
   */
  class DLB_COMMS_MEDIA_EXPORT identity {
   public:
     ...
     * @brief Compares the identity against another.
     * @param other another identity
     * @return true if the identity should be considered "less than" another one, for std containers, false otherwise.
     */
    bool operator<(const identity& other) const noexcept;
   ...
   *
   * This type is defined as a type alias, depending on the platform. It's an integer for Linux and Macos, and a string
   * on Windows. The native ID can be used to match the audio devices reported by the SDK against the system list of
   * devices.
   */
   ...
   * @return true if the other structure denotes the same device as this, false otherwise.
   */
  bool operator==(const audio_device& other) const { return pdata_ == other.pdata_; }

  /**
   * @brief Compare operator for comparison against other audio_devices' identity.
   * @param other The identity structure of the other device.
   * @return true if the other structure denotes the same device as this, false otherwise.
   */
  bool operator==(const audio_device::identity& other) const { return pdata_ == other; }

  /**
   * @brief Compare operator for comparison against other audio_devices' identity.
   * @param other The optional holding identity structure of the other device, or empty optional.
   * @return true if the argument is a non-empty optional, holding the identity structure which denotes the same device
...
struct DLB_COMMS_MEDIA_EXPORT hash<dolbyio::comms::audio_device::identity> {
  size_t operator()(const dolbyio::comms::audio_device::identity& obj) const noexcept;
};
}  // namespace std
