...
class DLB_COMMS_EXPORT local_video {
 public:
   ...
   */
  virtual async_result<void> start(const camera_device& device = {},
...
class DLB_COMMS_EXPORT remote_video {
 public:
   ...
   *
   * @note In the SDK 2.4.0, this API has been refactored. The older SDK versions allowed to only set up a single video
   * sink at a time, and the video sink would get all frames with their stream and track IDs. The older SDKs were passed
   * the sink by raw pointer, and the application was responsible for keeping the object alive until the new sink was
   * set. Now, the sink interface is passed in a shared pointer, and the SDK will drop its reference when the sink is
   * replaced or the track is removed. The applications migrating from older SDK versions should:
   * - install the event handler for the video_track_added event,
   * - in each video_track_added event create a new instance of the video_sink and use set_video_sink() for attaching
   * the sink.
   * - for the least intrusive port, the application may implement the video_sink interface as a proxy that bridges to
   * the old implementation of the video sink:
   * @code
   * class old_video_sink {
   * public:
   *   void handle_frame(const std::string&, const std::string&, std::unique_ptr<dolbyio::comms::video_frame>);
   * };
   *
   ...
   * public:
   *   new_video_sink(old_video_sink* old_sink, const std::string& stream_id, const std::string& track_id)
   *   : sink_(old_sink), stream_id_(stream_id), track_id_(track_id) {}
   *
   *   void handle_frame(std::unique_ptr<dolbyio::comms::video_frame> f) override {
   *     sink_->handle_frame(stream_id_, track_id_, std::move(f));
   *   }
   *
   * private:
   ...
   *     sdk->video().remote().set_video_sink(evt.track, std::make_shared<new_video_sink>(
   *       some_global_sink_pointer,
   *       evt.track.stream_id,
