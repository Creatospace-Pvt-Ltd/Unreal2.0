 ...
 * @ingroup default_injector
 */
class DLB_COMMS_ADDON_EXPORT injector : public video_frame_handler,
                                        public video_source,
                                        public audio_source,
                                        public std::enable_shared_from_this<injector> {
 public:
 ...
 private:
  ...
  std::shared_ptr<video_sink> sink() override { return {}; }
  void register_audio_frame_rtc_source(rtc_audio_source* source) override;
  void deregister_audio_frame_rtc_source() override;
  void set_sink(const std::shared_ptr<video_sink>& sink, const video_source::config& config) override;
 ...
 protected:
  ...
  std::shared_ptr<video_sink> video_sink_{};
...
class DLB_COMMS_ADDON_EXPORT injector_paced : public injector {
 public:
   ...
   * should be configured via the #set_video_frame_rate method. The video
  ...
  /**
   * @brief Sets the video frame rate in frames per second for injecting video frames into the
   * conference as a numerator/denominator pair. The pacing injector provides frames to WebRTC
   * at the interval specified by this method, provided the frames are available on the
   * queue.
   *
   * @param numerator The numerator of the frame rate value.
   * @param denominator The denominator of the frame rate value.
   */
 ...
 private:
  ...
  std::chrono::nanoseconds video_frame_interval_{33333333};
  /// @endcond
};
