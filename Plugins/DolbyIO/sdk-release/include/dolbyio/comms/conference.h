...
struct DLB_COMMS_EXPORT conference_info {
  ...
  /**
   * @since 2.4.0
   * @brief Indicates whether the conference represented by the object is audio only.
   */
  bool audio_only{};

  /**
   *  @since 2.4.0
   *  Indicates whether the conference represented by the object is a Dolby Voice conference.
   */
  bool dolby_voice{};

  /**
   * @since 2.4.0
   * @brief Indicates the possible recording formats the conference recording can be stored in.
   */
  ...
  /**
   * @since 2.4.0
   * @brief Indicates whether the conference is being recorded. If true the video recorded file shall be available
   * at the end of the call and downloadable immediately.
   */
  bool live_recording{};
};
...
struct DLB_COMMS_EXPORT conference_status_updated {
   ...
   *
   * The status like creating or created describe an inactive conference, but are not ended. The left, error or
   * destroyed statuses describe an ended conference. The ended conference may require cleanup on the application side,
   * depending on how the application interacts with the SDK events.
   *
   * @return true if the conference has ended, false if it's active or was never active.
   */
  bool is_ended() const {
    return status == conference_status::left || status == conference_status::error ||
...
/**
 * @brief Emitted when the conference starts or stops being recorded by the backend.
 * @ingroup conference_service
 * @ingroup events
 */
struct DLB_COMMS_EXPORT recording_status_updated {
  ...
  std::string conference_id; /**< The unique identifier of the conference. */
  std::string user_id;       /**< The unique identifier of the participant who started/stopped the recording. */
  recording_status status;   /**< The status of the recording update. */
  std::optional<long> start_timestamp; /**< Timestamp of when the recording was started. Optional is empty if
                                            status is not recording. */
};

/**
 * @brief Emitted periodically in conference to deliver participant audio levels.
 *
...
struct DLB_COMMS_EXPORT audio_levels {
  std::vector<dolbyio::comms::audio_level> levels; /**< The list of all active speakers with their audio levels. */
};
 ...
 public:
  ...
  struct conference_options {
    ...
    struct params {
      ...
      dolbyio::comms::video_codec video_codec = video_codec::h264; /**< The preferred video codec. */
      ...
      bool live_recording =
          false; /**< A boolean that indicates whether the conference is to be recorded on the backend. */
    };
  ...
  struct connection_options {
    ...
    std::optional<video_forwarding_strategy>
        forwarding_strategy; /**< Sets how the SDK should select conference participants whose videos will be
                              * transmitted to the local participant. There are two possible values; the selection can
                              * be either based on the participants' audio volume or the distance from the local
                              * participant. If this parameter is not set, the default value the last_speaker
                              * strategy.*/
   ...
   *
   * @param spatial_audio_style A spatial audio style to be used in the conference.
   *
   ...
   * video_frame_handler to the previous start call and would like to continue using this handler, you must pass the
  ...
  virtual async_result<void> start_screen_share(
      ...
      dolbyio::comms::screen_share_content_type type = dolbyio::comms::screen_share_content_type::detailed) = 0;
  ...
  /**
   * @brief Change the screen share content type if the screen share is already active. If screen share is not yet
   * active the content type is to be set during the #start_screen_share call.
   *
   ...
   *
   * @param callback The function object that is invoked when the forwarded video streams change.
   *
   * @returns The result object producing the #event_handler_id asynchronously.
   */
   ...
   *
   * @param callback The function object that is invoked periodically in the active conference.
   *
   * @returns The #async_result class that wraps a #solver that can contain the
   * #event_handler_id object.
   *           - SUCCESS: The #solver returns a void operator and the callback
   ...
   *
   * @param callback The function object that is invoked when the recording status event is received..
   *
   * @returns The #async_result class that wraps a #solver that can contain the
   * #event_handler_id object.
   *           - SUCCESS: The #solver returns a void operator and the callback
