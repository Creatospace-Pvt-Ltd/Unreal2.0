 ...
 public:
  ...
  static auto chain_value(low_level_solver<T, traits>* slv) {
    if constexpr (std::is_same_v<void, T>) {
      ...
      return [slv](T&& val) { slv->resolve(std::forward<T>(val)); };
    }
  }
};
/**
...
class DLB_COMMS_TEMPLATE_EXPORT low_level_solver : public low_level_solver_chain<traits> {
 ...
 public:
  ...
  void resolve(T&& res) {
    ...
    if (cb_) {
      cb_(std::forward<T>(res));
      reset();
    } else {
      result_ = std::forward<T>(res);
    }
  }
...
/// @cond DO_NOT_DOCUMENT
// Helper type to implement operations which do not necessrily need access to async_result's private parts, but can be
// expressed using the core async_result API:
template <typename traits>
struct async_result_helper {
  template <typename U>
  static async_result<void, traits> discard_result(async_result<U, traits>&& res) {
    if constexpr (std::is_same_v<U, void>) {
      return std::move(res);
    } else {
      return std::move(res).then([](U&&) {});
    }
  }

  static async_result<std::exception_ptr, traits> promote_error_to_async(async_result<void, traits>&& res) {
    return std::move(res)
        .then([]() -> std::exception_ptr { return {}; })
        .consume_errors([](std::exception_ptr&& e) -> std::exception_ptr { return std::move(e); });
  }

  template <typename U>
  static async_result<std::exception_ptr, traits> discard_result_and_get_error(async_result<U, traits>&& res) {
    return promote_error_to_async(discard_result(std::move(res)));
  }
};
...
class [[nodiscard]] async_result {
 ...
 public:
   ...
   */
  async_result(const low_level_solver_ptr<T, traits>& on_result, async_result_tags::low_level_tag)
      : state_(std::in_place_index_t<async_result_holds::solver>{}, on_result) {}
  ...
  async_result(TT&& val, async_result_tags::resolved_tag = async_result_tags::resolved)
      : state_(std::in_place_index_t<async_result_holds::value>{}, std::forward<TT>(val)) {}
  ...
  async_result(TT&& e, async_result_tags::failed_tag = async_result_tags::failed)
      : state_(std::in_place_index_t<async_result_holds::error>{}, std::forward<TT>(e)) {}
            ...
            [chained_slv = chained_slv.get(), err_cb{std::move(err_cb)}](std::exception_ptr&& e) mutable {
              try {
                ...
                if constexpr (void_result) {
                  ...
                  chained_slv->resolve(err_cb(std::move(e)));
                }
              } catch (...) {
 ...
 private:
  ...
  static std::enable_if_t<!std::is_same_v<void, TT>, bool> set_result_callback(
    ...
    return my_solver->set_callback([cb{std::forward<U>(cb)}, my_solver = my_solver.get()](TT&& val) mutable {
      auto wrapper_cb = [val{std::forward<TT>(val)}, &cb]() mutable { return cb(std::move(val)); };
  ...
  static void set_propagate_error_callback(const low_level_solver_ptr<T, traits>& from,
    ...
    from->set_propagate_error_callback([to = to.get()](std::exception_ptr&& err) { to->fail(std::move(err)); });
  }
...
/**
 * @brief Discard the result, consume errors and promote them to async path.
 *
 * This function is a helper function, using the core async_result API for constructing a new result, being a product of
 * several transformations of the original result: discarding its produced value, returning empty std::exception_ptr if
 * no errors happened, or consuming errors (in terms of using async_result::consume_errors() method) and returning
 * non-empty std::exception_ptr if errors happened.
 *
 * Use this method if a potential error produced by asynchronous operations needs handling asynchronously. Keep in mind,
 * that the product of this method is an asynchronous result which is always resolved with success, and the original
 * error can be extracted from the std::exception_ptr passed down the chain of operations in the value callback.
 *
 * @param res the result to convert into asynchronously produced exception pointer.
 * @return async_result holding the std::exception, which always resolves with success.
 */
template <typename T, typename traits>
inline async_result<std::exception_ptr, traits> discard_result_and_get_error(async_result<T, traits>&& res) {
  return async_result_helper<traits>::discard_result_and_get_error(std::move(res));
}
